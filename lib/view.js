...
exports.lookup = function(view, options){
  var viewClass = options.viewClass
    , orig = view = new viewClass(view, options)
  ...
  if (partial) {
    view = new viewClass(orig.prefixPath, options);
    if (!view.exists) view = orig;
  }

  // Try index ex: ./views/user/index.jade
  if (!layout && !view.exists) view = new viewClass(orig.indexPath, options);

  // Try ../<name>/index ex: ../user/index.jade
  // when calling partial('user') within the same dir
  if (!layout && !view.exists) view = new viewClass(orig.upIndexPath, options);

  // Try root ex: <root>/user.jade
  if (!view.exists) view = new viewClass(orig.rootPath, options);

  // Try root _ prefix ex: <root>/_user.jade
  if (!view.exists && partial) view = new viewClass(view.prefixPath, options);
...
res._render = function(view, opts, fn, parent, sub){
  ...
  // Get a reference to the `View` subclass to use
  options.viewClass = app.set('view class') || require('./view/fileView');
